---
title: 手把手带你用Linux socket 写个聊天室demo
date: 2019-08-14
tags: Linux/Socket
# description: sdf
---

## Linux 网络编程篇之 Tcp 协议介绍, C/S 通信及聊天室实现

基于 tcp 协议的网络程序流程图如下:

![tcp协议网络程序流程图](https://img-blog.csdnimg.cn/20181201111755889.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM2OTU4Mjg1,size_16,color_FFFFFF,t_70)

服务器调用 socket()、bind()、listen()完成初始化后，调用 accept()阻塞等待，处于监听端口的状态

客户端调用 socket()初始化后，调用 connect()发出 SYN 段并阻塞等待服务器应答

服务器应答一个 SYN-ACK 段，客户端收到后从 connect()返回，同时应答一个 ACK 段，服务器收到后从 accept()返回。
...
void *recvMsg(void *recvfd){
    ...
    while(1){
        if(recv(dealfd, recvbuf, MAXMSGSIZE, 0) == -1){
            fprintf(stderr, "Receive msg err: %s", strerror(errno));
        }
...
int main(int argc, char const *argv[])
{
    ...
    if( (serverfd = socket(AF_INET, SOCK_STREAM, 0) ) == -1){
        fprintf(stderr, "创建服务器套接字错误, %s", strerror(errno));
        exit(0);
    }
    ...
    if(bind(serverfd, (struct sockaddr *)&serveraddr, sizeof(struct sockaddr)) == -1){
        fprintf(stderr, "绑定套接字错误, %s", strerror(errno));
        exit(0);
    }

    if(listen(serverfd, 100) == -1){
        fprintf(stderr, "监听套接字错误, %s", strerror(errno));
    }

    printf("
Listening at %d port, wating connection.....", LOCALPORT);

    while(1){
        if((recvfd = accept(serverfd, (struct sockaddr *)&clientaddr, &sockleng)) == 0){
            fprintf(stderr, "连接错误, %s", strerror(errno));
            continue;
        }
        // 将该套接字描述符保存进数组
        clientFdarray[onLineNum++] = recvfd;

        printf("客户端套接字:%d 已开启", recvfd);
        ...
        if((perrno = pthread_create(&Precv, NULL, recvMsg, &recvfd)) != 0){
            fprintf(stderr, "创建子消息接收线程失败, %s", strerror(perrno));
            exit(perrno);
        }
    }
    close(serverfd);
...
int main(int argc, char const *argv[])
{
    ...
    printf("--------Successful connect to %s:%d--------", inet_ntoa(seraddr.sin_addr), ntohs(seraddr.sin_port));
    ...
    while(1){
        ...
        if(myRecvMsgHdr->tip == 0){
            fprintf(stdout, "        **用户 %d 加入聊天室 当前用户: %d 人**        ", myRecvMsgHdr->fd, myRecvMsgHdr->onLineNum);
        }else if(myRecvMsgHdr->tip == 1){
            printf("       **用户 %d 离开聊天室 当前用户: %d 人**         ", myRecvMsgHdr->fd, myRecvMsgHdr->onLineNum);
        }else if(myRecvMsgHdr->tip == 2){
            fprintf(stdout, "#%d> %s", myRecvMsgHdr->fd, recvbuf+sizeof(struct msgHdr));
        }
